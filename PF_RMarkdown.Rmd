...
```
Los datos del Informe Mundial de la Felicidad no necesitaron una limpieza debido a que estaban organizados de una forma que sirvió a la perfección para el análisis que se buscaba hacer. Sin embargo, se realizó un cambio de nombre a las variables de inglés a español para así poder tener todo el trabajo en un idioma, y también se eliminaron unas columnas que eran irrelevantes para el estudio que se quería realizar.
...
El análisis de los datos del Informe Mundial de la Felicidad reflejaron que el PIB per cápita tiene una alta correlación con la felicidad en cada país, además es la variable que tiene más relevancia en el puntaje de la felicidad. De hecho, en los gráficos se pudo observar que esta variable fue la que menor desviación estándar tiene, esto indica que la variación en los datos es menor que en las otras variables. 
